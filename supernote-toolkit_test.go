...
func TestMain(m *testing.M) {
	...
	out, err := exec.Command("go", "build", "-o", binPath, "./cmd/cli").CombinedOutput()
	if err != nil {
...
func setupA5X2Dir(t *testing.T) string {
	...
	copyFile(t, filepath.Join("test-files", "A5X2", "Standard.note"), filepath.Join(dir, "Standard.note"))

	sub := filepath.Join(dir, "sub")
	os.MkdirAll(sub, 0o755)
	copyFile(t, filepath.Join("test-files", "A5X2", "Artifacts.note"), filepath.Join(sub, "Artifacts.note"))
...
func TestCLIInvalidDevice(t *testing.T) {
	t.Parallel()
...
func TestCLI_A6X2(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			dir := t.TempDir()
			copyFile(t, filepath.Join("test-files", "A6X2", tt.file), filepath.Join(dir, tt.file))
			output := t.TempDir()
...
func TestCLI_A5X2_Horizontal(t *testing.T) {
	t.Parallel()

	dir := t.TempDir()
	copyFile(t, filepath.Join("test-files", "mixed", "horizontal_1090.note"), filepath.Join(dir, "horizontal_1090.note"))
	output := t.TempDir()
